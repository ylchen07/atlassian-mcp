...
func TestNewClientAddsHTTPS(t *testing.T) {
	...
	client, err := NewClient("example.atlassian.net", config.ServiceCredentials{
		Email:    "user",
		APIToken: "token",
	})
	if err != nil {
...
func TestNewClientWithContextPath(t *testing.T) {
	...
	client, err := NewClient("https://jira.example.com/jira", config.ServiceCredentials{
		Email:    "user",
		APIToken: "token",
	})
	if err != nil {
	...
	if client.BaseURL != "https://jira.example.com/jira" {
